...
def delete_patient_record(db, delete_option, delete_value):
    ...
    delete_appointments_query = "DELETE FROM appointments WHERE patient_id = %s"
    cursor.execute(delete_appointments_query, (delete_value,))

    if delete_option == "ID":
        cursor.execute("""select id from patients where id=%s""",(delete_value,))
        ...
        cursor.execute("""select contact_number from patients where contact_number=%s""",(delete_value,))
        result=cursor.fetchone()
        if result:
...
def insert_appointment_record(db, patient_id, appointment_date, appointment_time, doctor_name, notes):
    ...
    if result:
        ...
        db.commit()
        st.success("Appointment record inserted successfully.")
    else:
        st.error('No patients found')
...
def search_appointment(db):
    search_option = st.selectbox("Select search option", ["ID"],key="search_option")
...
def update_patient_record(db):
    ...
    if st.button("Search :magic_wand:"):
        ...
        if patient:
            ...
            df = pd.DataFrame([patient], columns=['ID', 'Name', 'Age', 'Contact Number', 'Address', 'Date Added','Email','doctor_name','disease','fee','tests','cnic'])
            st.dataframe(df)
...
def edit_patient(db):
    ...
    new_email = st.text_input("Enter new email", value=st.session_state.edit_patient[6])
    new_address = st.text_input("Enter new address", value=st.session_state.edit_patient[4])
...
def main():
    ...
    menu = ["Home","Add patient Record","Show patient Records", "Search and Edit Patient","Delete Patient Record",
            "Add patient Appointments","Show All Appointments","Edit Patient Appointments"]
    options = st.sidebar.radio("Select an Option :dart:",menu)
    st.sidebar.button("Logout",on_click=logouts)
    if options== "Home":
        st.subheader("Welcome to Patient Management System")
        st.write("Navigate from sidebar to access functions")
        st.image(gif_path)
       ...
       if st.button("add patient record"):
        ...
        if patients:
            ...
            df = pd.DataFrame(patients, columns=['ID', 'Name', 'Age', 'Contact Number', 'Address',  'date_added',' email','doctor_name','disease','fee','tests','cnic'])
            st.dataframe(df)
        else:
    ...
    elif options == "Delete Patient Record":
         st.subheader("Search a patient to delate :skull_and_crossbones:")
         delete_option = st.selectbox("Select delete option", ["ID", "Contact Number"], key="delete_option")
    ...
    elif options == "Edit Patient Appointments":
